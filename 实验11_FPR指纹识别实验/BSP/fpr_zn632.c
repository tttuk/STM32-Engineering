...
void FPR_DeleteFinger(void){
    ...
    printf("Input Finger ID to delete:\r\n");
    //简化实现：通过读取索引表找到已录入的指纹并提示
    ret = ZN632_ReadIndexTable();
    if(ret != 0){
        printf("Read Index Table Failed\r\n");
        return;
    }

    //此处可扩展为通过串口输入指定ID删除
    //uID = ...; //从串口获取
    //ret = ZN632_DeletChar(uID, 1);
    //if(ret == 0){
    //    ZN632_UnsetIndex(uID);
    //    printf("Delete Finger OK, ID:%d\r\n", uID);
    //}
}
