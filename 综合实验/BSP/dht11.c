 ...
 * 从DHT11读取一个字节，MSB先行
 */
static int16_t DHT11_ReadByte ( void ){
    uint8_t byte = 0;
    uint8_t i = 0;
    int8_t  bit;
...
int8_t DHT11_ReadData(DHT11_Data *pData){
    ...
    //开始接收数据
    {
        ...
        byte = DHT11_ReadByte(); if(byte < 0) return -1;
        pData->humi_int  = (uint8_t)byte;

        byte = DHT11_ReadByte(); if(byte < 0) return -1;
        pData->humi_deci = (uint8_t)byte;

        byte = DHT11_ReadByte(); if(byte < 0) return -1;
        pData->temp_int  = (uint8_t)byte;

        byte = DHT11_ReadByte(); if(byte < 0) return -1;
        pData->temp_deci = (uint8_t)byte;

        byte = DHT11_ReadByte(); if(byte < 0) return -1;
        pData->check_sum = (uint8_t)byte;
    }
