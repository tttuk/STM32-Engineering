...
volatile uint8_t  PN532_RxBuf[500];
volatile uint16_t PN532_RxBufLen = 0;
...
*/
void PN532_ClearRxBuf(void){
    memset((void *)PN532_RxBuf, 0, PN532_RxBufLen);
    PN532_RxBufLen = 0;
}

/*
轮询等待PN532响应就绪(ACK+数据帧全部到达)
timeout_ms: 超时时间(ms)，每10ms检查一次
返回0成功，-1超时
*/
static int8_t PN532_WaitForResponse(uint16_t timeout_ms){
    uint16_t elapsed = 0;
...
int8_t PN532_SAMConfiguration( uint8_t mode ){
    ...
    PN532_ClearRxBuf();  //发送前清空缓冲区
    PN532_SendData(data, sizeof(data));

    if(PN532_WaitForResponse(500) < 0){ ret = -1; goto _END; }
    ret = 0;
