 ...
 * @param  uX ：在特定扫描方向下该点的X坐标
 * @param  uY ：在特定扫描方向下该点的Y坐标
 * @note 可使用LCD_SetBackColor、LCD_SetTextColor、LCD_SetColors函数设置颜色
 * @retval 无
 */
...
uint16_t LCD_GetPixel( uint16_t usX, uint16_t usY ){
    ...
    if((usX < g_LCD_LenX) && (usY < g_LCD_LenY)){
        ...
        ILI9341_WriteCmd( 0x2E );   // read memory command
        usPixel = ILI9341_ReadData();  // dummy read
        usPixel = ILI9341_ReadData();
    }
    return usPixel;
}
 ...
 * @param  uX ：在特定扫描方向下字符的起始X坐标
 * @param  uY ：在特定扫描方向下该点的起始Y坐标
...
void LCD_DrawRectangle(uint16_t usX_Start, uint16_t usY_Start, uint16_t usWidth, uint16_t usHeight, uint8_t ucFilled){
    if(ucFilled){
        ...
        for(i = 0; i < usHeight; i++){
            LCD_DrawLine(usX_Start, usY_Start + i, usX_Start + usWidth - 1, usY_Start + i);
        }
    }
    else{
...
/**
 * @brief  在LCD上画圆 (Bresenham算法)
 * @param  ucFilled: 0=空心圆, 非0=实心圆
 */
void LCD_DrawCircle(uint16_t usX_Center, uint16_t usY_Center, uint16_t usRadius, uint8_t ucFilled){
    ...
    while(a <= b){
        if(ucFilled){
            LCD_DrawLine(usX_Center - b, usY_Center - a, usX_Center + b, usY_Center - a);
            LCD_DrawLine(usX_Center - a, usY_Center + b, usX_Center + a, usY_Center + b);
            LCD_DrawLine(usX_Center - a, usY_Center - b, usX_Center + a, usY_Center - b);
            LCD_DrawLine(usX_Center - b, usY_Center + a, usX_Center + b, usY_Center + a);
        }
        else{
            LCD_SetPixel(usX_Center - b, usY_Center - a);
            LCD_SetPixel(usX_Center + b, usY_Center - a);
            LCD_SetPixel(usX_Center - a, usY_Center + b);
            LCD_SetPixel(usX_Center + a, usY_Center + b);
            LCD_SetPixel(usX_Center - a, usY_Center - b);
            LCD_SetPixel(usX_Center + a, usY_Center - b);
            LCD_SetPixel(usX_Center - b, usY_Center + a);
