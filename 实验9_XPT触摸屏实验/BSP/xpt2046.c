...
/**
  * @brief  数值滤波函数：采样10次，去掉最大最小值，取平均值
  * @param  pCoord ：存放滤波后的ADC坐标值
  * @retval 0:成功, -1:失败
  */
static int8_t XPT2046_FilterXY( XPT2046_Coord *pCoord ){
...
/**
  * @brief  触摸屏校准函数
  * @param  pLCD: LCD坐标数组(3个点)
  * @param  pTouch: 触摸ADC坐标数组(3个点)
  * @param  pFactor: 输出的校准因子
  * @retval 0:成功
  */
int8_t XPT2046_CalibrationFactor(XPT2046_Coord *pLCD, XPT2046_Coord *pTouch, XPT2046_Factor *pFactor){
    // 三点校准法，使用三组(LCD坐标, 触摸ADC坐标)解出A,B,C,D,E,F
    // 此函数可由用户在校准时调用
    if(pLCD == NULL || pTouch == NULL || pFactor == NULL){
        return -1;
    }
    // 此处为校准函数框架，实际校准算法较长
    // 当前使用出厂默认校准参数
